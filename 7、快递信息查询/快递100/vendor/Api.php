...
class Express{
    ...
    public function Obtain($order='', $type=''){
        ...
        # 自动识别快递类型
        if (empty($type)) {
            $res  = json_decode($this->https_request('http://www.kuaidi100.com/autonumber/autoComNum?text='.$order), true);
            if (!$res || empty($res['auto'])) return $this->returnEcho('02', '快递类型自动识别识别');
        ...
        $res = json_decode($this->https_request('http://www.kuaidi100.com/query?type='.$type.'&postid='.$order), true);
        if (!$res || !$res['data']) return $this->returnEcho('04', '查询失败');
